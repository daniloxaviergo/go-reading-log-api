	...
	updateQuery := `
		UPDATE projects SET
			...
			days_unread = CASE WHEN started_at IS NOT NULL THEN 
				COALESCE((SELECT EXTRACT(DAY FROM NOW() - MAX(data))::INT FROM logs WHERE project_id = $1), 0)
			ELSE 0 END,
...
func GenerateRandomProject(id int64, seed int) *ProjectFixture {
	...
	// In production code, use math/rand with seed
	// For now, return deterministic data based on ID
	idInt := int(id)
